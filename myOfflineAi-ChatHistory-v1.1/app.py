...
# Creator: vbookshelf
# GitHub: https://github.com/vbookshelf/myOfflineAi-ChatHistory
# License: MIT
# Version: 1.1
#----------------------
...
TEMPERATURE = 0.4
TOP_P = 0.95
TOP_K = 50 # Not necessary to set this when using top_p
FREQUENCY_PENALTY = 0
REPEAT_PENALTY = 0
    ...
    <script type="module">
        ...
        function renderSavedChatsList(agentId) {
            ...
            chats.forEach(chat => {
                ...
                itemEl.className = 'history-item group p-3 bg-white rounded-lg cursor-pointer hover:bg-indigo-50 border border-slate-200';
                itemEl.innerHTML = `
                    <div class="flex justify-between items-start">
                        <div class="flex-grow overflow-hidden mr-2">
                            <p class="chat-title-display font-semibold text-slate-800 truncate">${chat.title}</p>
                            <input type="text" class="chat-title-input hidden w-full p-1 border border-indigo-300 rounded-md text-sm" value="${chat.title.replace(/"/g, '&quot;')}" />
                            <p class="text-xs text-slate-500 mt-1">${new Date(chat.timestamp).toLocaleString()}</p>
                        </div>
                        <div class="flex items-center opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                            <button class="edit-history-btn text-slate-500 hover:text-indigo-700 p-1" data-chat-id="${chat.id}">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg>
                            </button>
                            <button class="delete-history-btn text-red-500 hover:text-red-700 p-1" data-chat-id="${chat.id}">
                ...
                const editBtn = itemEl.querySelector('.edit-history-btn');
                const deleteBtn = itemEl.querySelector('.delete-history-btn');
                const titleDisplay = itemEl.querySelector('.chat-title-display');
                const titleInput = itemEl.querySelector('.chat-title-input');
                ...
                const stopEditing = async (save = false) => {
                    const newTitle = titleInput.value.trim();
                    const originalTitle = chat.title;

                    if (save && newTitle && newTitle !== originalTitle) {
                        try {
                            const res = await fetch(`/conversations/${agentId}/${chat.id}/title`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json' },
                ...
                itemEl.onclick = (e) => {
                    if (titleInput.classList.contains('hidden') && !e.target.closest('button')) {
...
def update_conversation_title(agent_id, chat_id):
    data = request.json
    ...
    if agent_id in conversations:
        ...
        if chat_index != -1:
            conversations[agent_id][chat_index]['title'] = new_title.strip()
            save_conversations(conversations)
            return jsonify({"status": "title updated"})
